...
収束的思考（CT）において、低難易度の前半（Test 1-3: Easy）と高難易度の後半（Test 4-6: Hard）で、要求される認知リソースがどのように変化するかを、WAIS-IV全指標（主要指標・下位検査・プロセス分析）を用いて検証した。
...
| 難易度 | WAIS詳細指標 | 手法 | 相関係数 ($\rho$) | $p$ 値 | 示唆される認知機能 |
|:---|:---|:---|:---|:---|:---|
| **CT Easy** | **行列推理 (Matrix)** | スピアマン | **0.474** | 0.0166 | 非言語的な法則性の把握 |
| **CT Hard** | **作動記憶 (WMI)** | スピアマン | **0.528** | 0.0067 | 総合的な情報の保持・操作 |
| **CT Hard** | **算数 (Arithmetic)** | スピアマン | **0.524** | 0.0071 | 複雑な論理的推論 |
| **CT Hard** | **数唱：逆唱 (Backward)** | スピアマン | **0.428** | 0.0326 | **高度な情報の能動的操作** |

#### 2. 可視化 (CT難易度 vs WAIS詳細相関ヒートマップ)
難易度上昇に伴い、相関の主役が「基礎的な推論能力」から「ワーキングメモリの操作側面」へと移行するプロファイルが確認された。

![CT難易度詳細ヒートマップ]<img width="2100" height="3000" alt="ct_difficulty_heatmap" src="https://github.com/user-attachments/assets/ac12f4a9-6155-425c-9ec4-f17b2a10eab6" />

#### 3. 考察
- **認知的要求の質的変化**: 低難易度のCT課題は「行列推理」に代表される流動性知能の基礎的な側面を反映している。一方、高難易度のCT課題（Hard）では、ワーキングメモリの指標（WMI、算数、逆唱）との相関が急激に強まる。
- **DTとの認知的共通基盤**: Step 13で見られた「DT独創性」と「逆唱」の相関と同様の結果が「CT高難易度」でも得られた。これは、**拡散思考の質的な高さと、収束思考の難易度への対応が、ともに『ワーキングメモリによる情報の能動的組み替え機能』を共通の認知的エンジンとしている**可能性を強く示唆している。
- **結論**: CT課題は難易度に応じて異なる認知機能を動員しており、高難易度設問における強いワーキングメモリ負荷が、被験者間の脳活動（効率化）の差を生む主要な要因であると考えられる。
