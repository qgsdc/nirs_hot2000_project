...
### 📊 サンプルサイズ (N数) の最終定義と管理
解析の整合性を保つため、以下の $N$ 数を厳格に適用する。

| 解析カテゴリー | サンプル数 (N) | 備考 |
|:---|:---:|:---|
| **基本母集団** | **25名** | 全26名から iwamoto 氏(有効セッション不足)を除外 |
| **LMM解析 (Step 21)** | **568試行** | 全580試行から HRV 外れ値 12 試行を除外した統合データ |
| **相関: DT脳活動 × WAIS** | **25名** | nakashima 氏の DT データは有効なため含める |
| **相関: CT脳活動 × WAIS** | **24名** | nakashima 氏を欠損値(QCによる全セッション除外)として除く |
| **相関: 課題成績(Score)のみ** | **25名** | 心理指標同士の比較には脳活動ノイズは影響しないため 25名 |

> **💡 忘れないためのルール**:
> 「脳の **CT** が絡むときだけ **24人**、それ以外は **25人**」と覚えておけば間違いありません。
