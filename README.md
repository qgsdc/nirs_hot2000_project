...
| **被験者単位の全除外** | **takatugi 氏** | 12 | 信号の安定性欠如および解析の一貫性確保のため |
...
#### 解析対象データの推移
最終的に、初期データの約90%にあたる 281 セッションを正味の解析対象とした。

| ステップ | セッション数 | 備考 |
|:---|:---|:---|
| **初期データ** | **312** | 26名 × 12セッション |
| **被験者全除外 (2名)** | **- 24** | iwamoto 氏, takatugi 氏の全データ |
| **個別判定除外 (3名分)** | **- 7** | nakashima(5), suga(1), kawaguchi(1) |
| **最終有効データ数** | **281** | **24名による解析母集団** |

> **Note**: `final_clean_data.mat` に格納されている `clean_data` は、上記除外プロセスをすべて完了した 281 セッションのみを保持している。

#### 除外基準
1. **セッション単位の除外**: 
   Step 5 で統計的な外れ値（Z-score ±3.0）として「Bad」判定されたセッションを自動削除。
2. **被験者単位の除外**: 
   有効なセッション数が極端に少なく、個人差の推定においてバイアスを生むリスクがある被験者を全削除（対象: iwamoto 氏, takatugi 氏）。
