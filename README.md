...
### Step 8: 線形混合モデル (LMM) による統計解析結果
`run_step8_lmm_preliminary.m` の実行結果（Plan B 採用）

血流の生理的遅延（5秒）を考慮し、直前15秒をベースラインとした解析により、以下の有意な脳活動が確認された。

| 比較内容 | 推定値 ($\beta$) | p-value | 判定 |
|:---|:---|:---|:---|
| **DT vs DT_Control** | **0.0231** | **< 0.001** | **有意な活動増 (***)** |
| **CT vs CT_Control** | **0.0137** | **0.0268** | **有意な活動増 (*)** |
| **DT vs CT (直接比較)** | 0.0009 | 0.8908 | 有意差なし |

**結論:**
拡散的思考(DT)および収束的思考(CT)のいずれにおいても、各制御課題と比較して前頭前野の有意な活性化が認められた。特にDTにおいて、極めて高い統計的有意性が示された。
